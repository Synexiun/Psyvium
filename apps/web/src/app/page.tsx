...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-console-900">
      ...
      {/* ── Hero — on the signature hairline data-grid ── */}
        ...
        <div className="container-vpsy relative py-20 md:py-28">
          ...
          <div className="mt-16 card p-6 shadow-console md:p-9">
            <div className="hairline-b mb-4 flex items-center justify-between pb-3">
              ...
              <span className="chip"><span className="me-1.5 inline-block h-1.5 w-1.5 rounded-full bg-teal animate-pulseline" aria-hidden />{t('common.live')}</span>
            </div>
      ...
      {/* ── Category ── */}
      <section id="category" className="hairline-t py-20">
        <div className="container-vpsy grid gap-10 md:grid-cols-[1fr_1.3fr] md:gap-16">
          ...
          <div className="card p-7">
            <p className="eyebrow text-signal">{L.panelEyebrow}</p>
            <ul className="mt-5 space-y-4">
              {L.questions.map((q, i) => (
                <li key={q} className="flex gap-4">
                  <span className="figure mt-0.5 text-xs text-teal">{String(i + 1).padStart(2, '0')}</span>
                  <span className="text-mist/85">{q}</span>
                </li>
              ))}
            </ul>
            <p className="hairline-t mt-6 pt-5 text-sm text-mist/50">{L.panelFoot}</p>
          </div>
        </div>
      </section>

      {/* ── Six layers ── */}
      <section id="layers" className="hairline-t py-20">
        <div className="container-vpsy">
          ...
          <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {L.layers.map((l) => (
              <article key={l.n} className="card group p-6 transition hover:border-teal/40">
      ...
      {/* ── Intelligence — hairline grid of agents ── */}
      <section id="intelligence" className="hairline-t py-20">
        <div className="container-vpsy">
          ...
          <div className="mt-12 grid gap-px overflow-hidden rounded-md border border-line/15 bg-line/15 md:grid-cols-2">
            {L.agents.map((a) => (
              <div key={a.t} className="bg-console-800 p-6 transition hover:bg-console-700/70">
      ...
      {/* ── Governance ── */}
      <section id="governance" className="hairline-t py-20">
        <div className="container-vpsy">
