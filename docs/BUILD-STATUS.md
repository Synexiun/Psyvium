...
>
> **Clinician-assigned assessment wave (2026-07-16):** the standard-instrument assign→complete→review→AI-briefing loop shipped (commit `319aa5a`). A `AssessmentAssignment` context (RLS-forced table + migration) lets a clinician push a published instrument onto a caseload client's dashboard; the client completes it as a calm static form (score/band/interpretation **suppressed** — they hold `ASSESSMENT_ADMINISTER`, not `ASSESSMENT_INTERPRET`); completion runs the exact deterministic batch pipeline (bands + safety-item→Risk) behind a **CAS-first status guard** (double-submit can't double-score) and fires a governed AI briefing (`AIRecommendation`, PENDING) that never blocks the client. Standard pack seeded: 10 public-domain instruments with scoring keys + clinician interpretation guides. Web: role-aware `/assessments`, `/home` pending card, `assess:` namespace across all 10 locales. Docs: `07-psychometrics-engine.md` §14. Verified: API + web typecheck, 15/15 assignment tests, full web build (25/25 pages, all 10 dicts), migration CI-ready.
>
...
| 16 | Psychometrics | 4 | ✅ | Classical + IRT EAP + CAT; safety-item → Risk; **InstrumentLicenseGrant** 403 gate; ItemTranslation provenance; **unified caseload ABAC on every path** (CAT start/answer/read included; manager minimum-necessary parity with ClinicalAccessService — audit 30-day item 5 closed 2026-07-15). **Clinician-assigned workflow (2026-07-16)**: standard instrument pack (10 with scoring keys + clinician guides) + `AssessmentAssignment` context — clinician assigns → client completes from dashboard (score-suppressed) → clinician reviews answers/score/band/guide + governed AI briefing; CAS-first no-double-score guard; 15 unit tests; doc §14 |
