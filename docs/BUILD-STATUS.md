...
- **Quality gates** — full turbo build 4/4; **647 API tests** (incl. blocking clinical-safety suite); **real API lint gate** (ESLint 9, type-aware async-correctness rules, `--max-warnings 0` — replaced the `echo 'lint ok'` stub the audit scored 3/10); live end-to-end smokes per wave.
